...
""" 生成核密度矩阵 （耗时长） """
def build_matrix_3d(population):
...
""" 生成核密度矩阵 （耗时长） """
def build_matrix_1d(population):
    time_start = time.time()
    a1 = []
    a2 = []
    a3 = []
    ...
    vmin = m3.min()
    vmax = m3.max()
    V = np.mgrid[vmin : vmax : 32j]    # 复数的步长代表分段数

    positions = np.vstack([V.ravel()])
    values = np.vstack([m3])
    kernel = stats.gaussian_kde(values)
    Z = np.reshape(kernel(positions).T, V.shape)

    time_end = time.time()
    print('size={}, totally cost {}s'.format(len(population), time_end - time_start))

    kde = [kernel([p[3]])[0] for p in population]
    m4 = np.asarray(kde)
    indexes = [p[0] for p in population]
    mid = np.asarray(indexes)
...
if __name__ == "__main__":
    ...
    matrix = build_matrix_1d(population)
